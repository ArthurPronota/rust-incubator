...
В Rust закрепление не является автоматически транзитивным. Если у вас есть `Pin<&mut Parent>`, поля Parent не закрепляются автоматически. Вы всё ещё можете перемещать поля из Parent, если не решите рассматривать эти поля как «структурно закрепленные».
