...
1. Вы создаете структуру Admin и реализуете Deref, чтобы она возвращала &User. Теперь вы можете вызывать методы User прямо на объекте Admin. Это кажется удобным, но Admin — это не указатель на пользователя, это отдельная сущность.
