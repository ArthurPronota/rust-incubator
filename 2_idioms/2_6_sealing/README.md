...
<a name="q-020601"><h3>Полное руководство по закрытым свойствам (sealed traits) в Rust</h3></a>

Это перевод статьи [Predrag Gruevski: A definitive guide to sealed traits in Rust][4]
...
||нисходящий код может использовать его в качестве границы|нисходящий код может вызывать его методы|нисходящие типы могут его реализовывать|
|---|----|----|---|
|pub trait|✅|✅|✅|
...
- [`Полная матрица возможностей`](#полная-матрица-возможностей)
...
> В настоящее время в упомянутом выше коде ошибок нет.
...
Описанные здесь методы позволяют нам точно выбрать ту комбинацию переопределяемых и вызываемых методов, которую мы хотим реализовать в наших трейтах:
...
|некоторые методы переопределяемые|✅[по умолчанию сигнатурно запечатанные методы + pub fn чтобы вызвать их](https://play.rust-lang.org/?version=stable&mode=debug&edition=2021&gist=fcdedb4be8f688e36dc07e16df28c080) + ["final methods" pre-RFC](https://internals.rust-lang.org/t/pre-rfc-final-trait-methods/18407)|✅частично запечатанные / ["final methods" pre-RFC](https://internals.rust-lang.org/t/pre-rfc-final-trait-methods/18407)|❌|
|трейт не может быть реализованным нижестоящим (никакие методы не могут быть переопределены)|✅ (запечатаны supertrait)|✅(как минимум один метод с запечатаной сигнатурой, не имеющий реализации по умолчанию)|✅(все методы сигнатурно запечатаны)|

Я освоил эти методы, читая малоизвестные темы на GitHub, посты о внутренней структуре Rust и обсуждения на форуме Zulip, а также благодаря множеству кропотливых экспериментов.

Мне нужно было убедиться, что я понимаю весь спектр вариантов реализации трейтов, потому что [cargo-semver-checks](https://crates.io/crates/cargo-semver-checks) не должны давать неверные советы. Роль линтеров заключается в том, чтобы взять опыт автора и сделать его общедоступным — а для этого сначала нужно было приобрести некоторые знания в области блокировки трейтов!

Я многому научился из содержательных постов в блоге Rustaceans, и я стараюсь передать этот опыт другим! Надеюсь, этот пост избавит будущих пользователей Rustaceans от необходимости так же глубоко копать, как мне 🦀
Спасибо Тиму Макнамаре, Алоне, bernardo_amc, arriven, Луке Пальмиери и Фарнцу за отзывы на черновики этого поста. Все ошибки — мои собственные.
...
<a name="q-020602"><h3>Что означает «запечатывание» в программировании в широком смысле?</h3></a>
...
<a name="q-020603"><h3>Что такое «запечатывание трейтов» в Rust? Когда это полезно?</h3></a>
...
<a name="q-020604"><h3>Какие ограничения имеет механизм изоляции трейтов в [Rust]? Что он мог бы обеспечить, если бы поддерживался компилятором?</h3></a>
