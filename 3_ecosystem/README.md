...
- [Почему многопоточность необходима для программ на Rust и какие проблемы она решает? Чем отличается многопоточность от параллельной обработки? Как можно распараллелить код на Rust?][33]
- [Что такое асинхронность и какие проблемы она решает? Чем она отличается от параллельного выполнения потоков? Какое решение для асинхронности предлагает Rust и почему он имеет именно такую ​​архитектуру?][34]
- [Что такое акторы? Когда они полезны?][35]
...
```mermaid
flowchart TD
    A[Начало download_img] --> B[Запись start time]
    B --> C{path_to_img_is_url?}
    
    C -->|Да - URL| D[Создание HTTP клиента]
    D --> E[Выполнение GET запроса]
    E --> F{Статус 200-299?}
    F -->|Нет| G[Возврат ошибки]
    
    F -->|Да| H{rate_limit == 0?}
    H -->|Да| I[Загрузка без ограничений: resp.bytes]
    H -->|Нет| J[Загрузка с rate limiting]
    
    J --> J1[Создание stream из ответа]
    J1 --> J2[Инициализация downloaded=0, all_data=Vec]
    J2 --> J3{Получение chunk из stream}
    J3 -->|Есть данные| J4[Добавление chunk в all_data]
    J4 --> J5[Расчет elapsed time]
    J5 --> J6[Расчет expected_bytes по скорости]
    J6 --> J7{downloaded > expected_bytes?}
    J7 -->|Да| J8[tokio::time::sleep]
    J8 --> J3
    J7 -->|Нет| J3
    
    J3 -->|Нет данных| J9[Возврат all_data]
    
    C -->|Нет - файл| K[tokio::fs::read файла]
    
    I --> L[Определение формата изображения]
    J9 --> L
    K --> L
    
    L --> M{Формат JPEG или PNG?}
    M -->|Нет| N[Возврат ошибки формата]
    
    M -->|Да| O[Удаление метаданных]
    O --> P{img_quality == 100?}
    
    P -->|Нет - нужно сжатие| Q{Формат?}
    Q -->|JPEG| R[Сжатие через mozjpeg]
    Q -->|PNG| S[Сжатие через oxipng]
    
    R --> T[Создание хеша SHA256 пути]
    S --> T
    P -->|Да| T
    
    T --> U[Формирование имени выходного файла]
    U --> V[Создание выходного файла]
    V --> W[Запись данных в файл]
    W --> X[Логирование времени обработки]
    X --> Y[Возврат Ok]
...
### Что такое асинхронность и какие проблемы она решает? Чем она отличается от параллельного выполнения потоков? Какое решение для асинхронности предлагает Rust и почему он имеет именно такую ​​архитектуру?
...
### Что такое акторы? Когда они полезны?
...
[32]: https://github.com/ArthurPronota/rust-incubator/tree/main/3_ecosystem#что-следует-использовать-для-создания-интерфейса-командной-строки-cli-на-rust-как-организовать-конфигурацию-для-моего-приложения-и-зачем
[33]: https://github.com/ArthurPronota/rust-incubator/tree/main/3_ecosystem#почему-многопоточность-необходима-для-программ-на-rust-и-какие-проблемы-она-решает-чем-отличается-многопоточность-от-параллельной-обработки-как-можно-распараллелить-код-на-rust
[34]: https://github.com/ArthurPronota/rust-incubator/tree/main/3_ecosystem#что-такое-асинхронность-и-какие-проблемы-она-решает-чем-она-отличается-от-параллельного-выполнения-потоков-какое-решение-для-асинхронности-предлагает-rust-и-почему-он-имеет-именно-такую-архитектуру
[35]: https://github.com/ArthurPronota/rust-incubator/tree/main/3_ecosystem#что-такое-акторы-когда-они-полезны