...
После выполнения этих заданий вы сможете ответить на следующие вопросы:
 ...
 - [Какие возможности тестирования предлагает Rust и когда их следует использовать? Почему следует придерживаться стиля BDD?][030001]
- [Что такое макросы? Чем они отличаются? Какие преимущества дает их использование? Когда следует писать макрос?][030002]
- [Как работать с датой и временем в Rust? Как хранить время? Как передавать его другим приложениям?][030003]
- [Как используются регулярные выражения в Rust? Когда их недостаточно? Как написать собственный парсер в Rust?][030004]
- [В чём заключаются различия между итераторами и коллекциями в Rust? Каково назначение неизменяемых коллекций? Почему важно использовать параллельные коллекции?][030005]
- [Что следует использовать для сериализации в Rust? Почему это хорошо или плохо?][030006]
- [Как генерировать случайные числа в Rust? Какие гарантии генератора случайных чисел следует выбирать и когда?][030007]
- [Что следует использовать для хеширования паролей в Rust? Как зашифровать сообщение с помощью Rust? Как следует сравнивать секретные значения и почему?][030008]
- [Как организовано логирование в экосистеме Rust? Почему мне следует интересоваться структурированным логированием?][030009]
- [Что следует использовать для создания интерфейса командной строки (CLI) на Rust? Как организовать конфигурацию для моего приложения и зачем?][030010]
- [Почему многопоточность необходима для программ на Rust и какие проблемы она решает? Чем отличается многопоточность от параллельной обработки? Как можно распараллелить код на Rust?][030011]
- [Что такое асинхронность и какие проблемы она решает? Чем она отличается от параллельного выполнения потоков? Какое решение для асинхронности предлагает Rust и почему он имеет именно такую ​​архитектуру?][030012]
- [Что такое акторы? Когда они полезны?][030013]
...
Требования:
- Принимает на вход список файлов и удаленных [URL]-адресов либо через [аргументы CLI], [STDIN], либо считывает его из указанного файла (разделенного [EOL]).
- Позволяет настроить количество обрабатываемых изображений одновременно.
- Позволяет настроить выходной каталог для хранения обработанных изображений.
- Позволяет настроить качество вывода обрабатываемых изображений в формате [JPEG].
- Считывайте конфигурацию с возрастающим приоритетом из: файла (формат на ваш выбор), [переменных среды][22], [CLI] аргументов. Все это необязательно указывать.
- Поддержка переменной окружения `RUST_LOG`, позволяющей детально настраивать уровни ведения журнала для каждого модуля.
- Выводите время выполнения в логи, чтобы было легко увидеть, сколько времени занимает та или иная операция во время выполнения.

Если у вас будет достаточно времени после внедрения базовых требований, подумайте о том, чтобы добавить в свое решение следующее:
 - Позволяет настроить ограничение скорости загрузки изображений с удаленных [URL] адресов.
 - Опишите свою реализацию с помощью модульных тестов и тестов E2E.
 - Также поддерживается изображения в формате [PNG].
 - Добавьте к своему коду исчерпывающую документацию.
 
#### Блок схема работы программы:

```mermaid
flowchart TD
    A[Начало работы программы] --> B[Инициализация: env_logger::init]
    B --> C[Получение аргументов CLI: args::get_args]
    C --> D[Загрузка конфигурации: conf_load::load_config]
    
    D --> E{Успешно?}
    E -->|Нет| F[Логирование ошибки]
    F --> G[process::exit 1]
    
    E -->|Да| H[Получение списка изображений: args::get_list_all_images]
    H --> I{Успешно?}
    I -->|Нет| J[Логирование ошибки]
    J --> G
    
    I -->|Да| K{Список пуст?}
    K -->|Да| L[Логирование 'No images']
    L --> G
    ...
    P --> Q[Сбор результатов в вектор]
    Q --> R[Обработка результатов каждого изображения]
    
    R --> S{Проверка результата}
    S -->|OkOk| T[Успешная загрузка]
    S -->|OkErr| U[Логирование ошибки загрузки]
    S -->|Err| V[Логирование ошибки выполнения]
    
    T --> W[Продолжить цикл]
    U --> W
    V --> W
    
    W --> X{Все результаты обработаны?}
    X -->|Нет| R
    X -->|Да| Y[Логирование общего времени]
    Y --> Z[Конец программы]
```

<hr>

<a name="q-030001"><h3>Какие возможности тестирования предлагает Rust и когда их следует использовать? Почему следует придерживаться стиля BDD?</h3></a>
...
<a name="q-030002"><h3>Что такое макросы? Чем они отличаются? Какие преимущества дает их использование? Когда следует писать макрос?</h3></a>
...
<a name="q-030003"><h3>Как работать с датой и временем в Rust? Как хранить время? Как передавать его другим приложениям?</h3></a>
...
<a name="q-030004"><h3>Как используются регулярные выражения в Rust? Когда их недостаточно? Как написать собственный парсер в Rust?</h3></a>
...
<a name="q-030005"><h3>В чём заключаются различия между итераторами и коллекциями в Rust? Каково назначение неизменяемых коллекций? Почему важно использовать параллельные коллекции?</h3></a>
...
<a name="q-030006"><h3>Что следует использовать для сериализации в Rust? Почему это хорошо или плохо?</h3></a>
...
<a name="q-030007"><h3>Как генерировать случайные числа в Rust? Какие гарантии генератора случайных чисел следует выбирать и когда?</h3></a>
...
<a name="q-030008"><h3>Что следует использовать для хеширования паролей в Rust? Как зашифровать сообщение с помощью Rust? Как следует сравнивать секретные значения и почему?</h3></a>
...
<a name="q-030009"><h3>Как организовано логирование в экосистеме Rust? Почему мне следует интересоваться структурированным логированием?</h3></a>
...
#### 3. Почему это важно для вас?
   ...
   1. Масштабируемый поиск: Если у вас миллионы строк логов, вы не сможете эффективно искать по ним через grep. В системах вроде ELK (Elasticsearch) или Loki вы сможете мгновенно отфильтровать логи по полю user_id, так как оно индексируется отдельно от текста [2.1].
   2. Аналитика из логов: Вы можете строить графики прямо по логам (например, среднее время ответа API), если записываете latency_ms как числовое поле.
   3. Контекстное отслеживание (Spans): В асинхронном Rust (tracing) структурированные логи позволяют связать все события одного HTTP-запроса общим request_id, даже если они перемешаны с логами других запросов.
   4. Типобезопасность: Вы передаете объекты (числа, булевы значения), а не форматируете их в строки вручную. Это быстрее и исключает ошибки форматирования.

#### Сводная таблица выбора

|Характеристика|Обычные логи (log)|Структурированные (tracing)|
|--------------|------------------|---------------------------|
|Для чего|Простые CLI, библиотеки.|Веб-сервисы, микросервисы.|
|Формат|Текст для человека.|JSON/Данные для машин.|
|Сложность|Очень низкая.|Средняя (нужна настройка слоев).|
|Главный плюс|Скорость разработки.|Мощная диагностика в продакшене.|

__Итог:__ Для любого серьезного проекта выбирайте tracing. Это обеспечит вашему приложению отличную наблюдаемость (Observability).

<hr>

<a name="q-030010"><h3>Что следует использовать для создания интерфейса командной строки (CLI) на Rust? Как организовать конфигурацию для моего приложения и зачем?</h3></a>

Для создания профессионального CLI-интерфейса и надежной системы конфигурации в Rust существует «золотой стандарт» инструментов, обеспечивающих типобезопасность и удобство.

#### 1. Создание CLI: Clap (Command Line Argument Parser)

 Это самая популярная библиотека для парсинга аргументов. Рекомендуется использовать её derive-режим, который позволяет описывать интерфейс через обычные структуры.

 - Преимущества: Автоматическая генерация --help, проверка типов (например, если ожидается число, clap выдаст ошибку при вводе текста), поддержка вложенных команд (subcommands).
...
#[derive(Parser)]
#[command(version, about = "Мое приложение")]
struct Cli {
...
Для управления настройками лучше всего использовать связку Serde (для парсинга) и Config-rs (для объединения источников).

Как организовать иерархию (Priority Order):

Профессиональные приложения в загружают настройки в следующем порядке (каждый следующий слой перекрывает предыдущий):
   1. Значения по умолчанию (зашиты в коде).
   2. Файл конфигурации (config.toml, settings.json).
   3. Переменные окружения (ENV vars — критично для Docker/Kubernetes).
   4. Аргументы командной строки (CLI flags — имеют наивысший приоритет).

Зачем так сложно?

- Безопасность: Секреты (пароли, API-ключи) нельзя хранить в файлах. Их передают через переменные окружения.
- Гибкость: Вы можете запустить один и тот же бинарный файл на сервере (с ENV-настройками) и локально (с файлом), не меняя код.
- Fail-Fast: Типобезопасная конфигурация проверяет корректность всех данных при запуске. Приложение упадет сразу, если в порту указаны буквы, а не число.

#### 3. Сводная таблица инструментов

|Задача|Крейт (Библиотека)|Почему это важно?|
|------|------------------|-----------------|
|CLI|Парсинг|Clap|Валидация аргументов и авто-документация.|
|Десериализация|Serde|Превращает текст (TOML/JSON) в структуры Rust.|
|Слияние конфигов|Config|Объединяет файлы, ENV и дефолты.|
|Переменные .env|Dotenvy|Удобно для локальной разработки.|
...
<a name="q-030011"><h3>Почему многопоточность необходима для программ на Rust и какие проблемы она решает? Чем отличается многопоточность от параллельной обработки? Как можно распараллелить код на Rust?</h3></a>

В Rust многопоточность и параллелизм — это основные способы выжать максимум производительности из современного многоядерного железа.

#### 1. Почему многопоточность необходима

Современные процессоры больше не становятся быстрее за счет тактовой частоты; они просто добавляют больше ядер. Чтобы программа работала быстрее, она должна делать несколько дел одновременно.

Проблемы, которые она решает:

- Отзывчивость: GUI или веб-сервер остаются активными, пока фоновый поток выполняет тяжелую работу.
- Пропускная способность: Обработка 100 запросов одновременно вместо одного за другим.
- Использование ресурсов: Использование всех 16 ядер процессора вместо того, чтобы оставлять 15 из них без дела.

#### 2. Многопоточность vs Параллельная обработка

Эти термины часто путают, но у них разные цели:

|Характеристика|Многопоточность (Конкурентность)|Параллельная обработка (Параллелизм)|
|--------------|--------------------------------|-------------------------|
|Цель|Управление: Работа со множеством задач сразу.|Скорость: Выполнение одной задачи быстрее.|
|Аналогия|Один повар готовит суп и салат (переключаясь).|Два повара готовят суп и салат (одновременно).|
|Использование CPU|Может работать на одном ядре (разделение времени).|Требует наличия нескольких физических ядер.|
|Типичное использование|Ожидание сети/диска (I/O).|Тяжелая математика, обработка видео (CPU-bound) [2.1].|

#### 3. Как распараллелить код на Rust

Rust обеспечивает «бесстрашный параллелизм» благодаря модели владения и трейтам Send/Sync, которые предотвращают состояние гонки (data races) еще на этапе компиляции.

   А. Параллелизм данных: Rayon
 
   Золотой стандарт. Он превращает обычные итераторы в параллельные почти без усилий.

   - Сценарий: У вас есть большой Vec, и вы хотите обработать каждый элемент.
   - Пример: 
```rust
list.par_iter().map(|x| compute(x)).collect();
```

   Б. Низкоуровневые потоки: std::thread

   Запускает реальные потоки ОС.

   - Сценарий: Долгоживущие фоновые задачи (например, агент мониторинга).
   - Примечание: Создание потока — дорогая операция; для мелких задач используйте пул потоков.

   В. Асинхронная конкурентность: Tokio

   Это не совсем параллелизм, но позволяет обрабатывать миллионы задач ввода-вывода (I/O) на нескольких потоках.

   Сценарий: Веб-серверы, прокси, драйверы баз данных.

   Сводная таблица

   |Если ваша задача...|Используйте...|
   |-------------------|--------------|
   |Нагружает CPU (Математика, Сортировка)|Rayon|
   |Нагружает I/O (Сеть, API вызовы)|Tokio|
   |Фоновый сервис|std::thread|
   |Общее состояние (Кэш, Карта)|[DashMap](https://docs.rs/dashmap/latest/dashmap/)|

   Совет профи: Компилятор Rust остановит вас, если вы попытаетесь передать данные между потоками небезопасно. Если вы видите ошибку о том, что Rc не является Send, вам, скорее всего, нужно перейти на Arc.

   Примечание:

   __DashMap__ — это стандарт де-факто для создания высокопроизводительных параллельных хеш-таблиц. 

   Если коротко: это HashMap, которую можно безопасно и быстро использовать из множества потоков одновременно без необходимости оборачивать её в громоздкий `Arc<Mutex<...>>`.

<hr>

<a name="q-030012"><h3>Что такое асинхронность и какие проблемы она решает? Чем она отличается от параллельного выполнения потоков? Какое решение для асинхронности предлагает Rust и почему он имеет именно такую ​​архитектуру?</h3></a>


Асинхронность в Rust — это способ написания программ, которые могут эффективно управлять тысячами одновременных задач (особенно связанных с вводом-выводом), не перегружая операционную систему.

#### 1. Что такое асинхронность и какие проблемы она решает?

__Асинхронность__ — это парадигма программирования, при которой выполнение задачи может быть приостановлено (без блокировки потока), пока она ожидает внешнего события (ответа от базы данных, сетевого пакета или таймера).

Проблемы, которые она решает:

- __Проблема C10k (10 000 соединений):__ Обычные потоки ОС слишком «тяжелые» (каждый потребляет ~2 МБ стека). Создать 10 000 потоков невозможно — память закончится раньше. Асинхронность позволяет обрабатывать эти 10 000 соединений на паре сотен мегабайт ОЗУ.
- __Простой ресурсов:__ Пока программа ждет ответа от API, процессор не «спит», а переключается на выполнение другой задачи.
- __Масштабируемость:__ Позволяет одному серверу обрабатывать на порядки больше запросов, чем при классическом многопоточном подходе.

#### 2. Чем она отличается от параллельного выполнения (Threads)?

Многие путают эти понятия, но в Rust между ними четкая граница:

|Характеристика|Параллелизм (Threads/Rayon)|Асинхронность (Async/Tokio)|
|--------------|---------------------------|---------------------------|
|Суть|Одновременное выполнение кода.|Конкурентное управление задачами.|
|Для чего|Вычисления (CPU-bound): математика, видео.|Ожидание (I/O-bound): сеть, диски, БД.|
|Потоки ОС|Создает реальные системные потоки.|Работает внутри небольшого пула потоков.|
|Управление|Управляется планировщиком ОС.|Управляется рантаймом (библиотекой).|
|Стоимость|Высокая (Context Switch через ядро).|Нулевая (Zero-cost переключение в коде).|

#### 3. Какое решение предлагает Rust? (Future)

Решение Rust базируется на Future и модели опроса (Pull-based model).

Почему архитектура именно такая?

В отличие от JavaScript или Go, где рантайм (движок) встроен в язык, в Rust:

   1. Нет встроенного рантайма: Вы сами выбираете исполнителя (например, Tokio или async-std). Это позволяет использовать Rust даже в микроконтроллерах без ОС.
   2. Нулевая стоимость (Zero-cost): Асинхронная функция превращается компилятором в компактную Машину состояний (State Machine). Она занимает в памяти ровно столько места, сколько нужно для хранения локальных переменных.
   3. Ленивость (Lazy): В Rust асинхронный код ничего не делает, пока вы не примените к нему .await. Это дает вам полный контроль: вы можете создать 1000 задач и запустить их только тогда, когда вам удобно.

Как это работает:

- async: Помечает блок кода как машину состояний (Future).
- .await: Точка приостановки. Если данные не готовы, управление возвращается планировщику.
- Waker: Механизм, который «будит» задачу, когда приходят данные из ОС, чтобы рантайм снова вызвал её метод poll().

#### Итог

Используйте параллелизм (__Rayon__), если вам нужно ускорить вычисления. Используйте асинхронность (__Tokio__), если ваше приложение много ждет (сетевые запросы, база данных).

<hr>

<a name="q-030013"><h3>Что такое акторы? Когда они полезны?</h3></a>

В Rust акторы — это архитектурный паттерн, где «Актор» является независимой единицей исполнения, которая владеет своими данными и общается с другими частями программы исключительно через передачу сообщений.

Представь актора как маленькое закрытое государство: никто не может зайти и потрогать его внутренности (переменные), можно только отправить официальное письмо (сообщение) в почтовый ящик (mailbox).

#### Как это работает в Rust?

1. Изоляция состояния: Внутри актора нет Mutex или RwLock. Данные изменяются последовательно при обработке очереди писем. Это избавляет от состояния гонки (data races).
2. Почтовый ящик: У каждого актора есть канал (обычно mpsc или bounded).
3. Адрес (Addr/Recipient): Вы не держите в руках сам объект актора, у вас есть только его адрес. Вы «кидаете» сообщение в адрес и идете дальше (async).

#### Когда они полезны?

- __Сложное состояние без блокировок:__ Когда у вас есть данные, к которым обращаются сотни потоков. Вместо того чтобы все они дрались за один Mutex, они просто шлют сообщения актору, который обрабатывает их по очереди без задержек на блокировку.
- __Чат-серверы и игровые объекты:__ Идеально, когда каждый пользователь или монстр — это отдельный актор со своим здоровьем, инвентарем и логикой.
- __Отказоустойчивость:__ Если один актор «упал» (panic), его супервизор (родительский актор) может его просто перезапустить. Это основа философии «Let it crash» из Erlang, перенесенная в Rust.
- __Микросервисы внутри монолита:__ Акторы позволяют разбивать приложение на слабосвязанные части, которые легко тестировать по отдельности.

#### Чем пользоваться в 2026 году?

- Actix — де-факто стандарт. Очень быстрый, мощный, но имеет свой специфический рантайм.
- Xactor — более современный и легкий, базируется на Tokio.
- Собственная реализация: Часто в Rust актора делают просто через tokio::spawn и бесконечный цикл loop { select! { ... } } с каналами. Это часто называют «актором на минималках».

#### Сравнение с обычными методами

|Подход|Как меняем данные?|Риск Deadlock|
|------|------------------|-------------|
|Shared State|Arc<Mutex<T>>|Высокий|
|Actors	Сообщения (Messages)|Низкий (но возможен цикл сообщений)|

__Итог:__ Используй акторов, если твоя логика напоминает живой организм с кучей независимых участников, которые постоянно обмениваются информацией. Если тебе нужно просто быстро сложить числа в массиве — используй Rayon.
