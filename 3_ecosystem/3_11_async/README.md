...
Программа должна прочитать список ссылок из файла `<file>`, а затем одновременно загрузить содержимое каждой ссылки в отдельный файл `.html` (названный по ссылке).

Аргумент `--max-threads` должен контролировать максимальное количество одновременно работающих потоков в программе (по умолчанию должно соответствовать количеству процессоров).

_Примечание:_

1. В контексте Cargo (менеджера пакетов Rust) опция -p является сокращением от --package.

    Эта опция указывает Cargo, какой именно пакет (проект) нужно запустить, если вы находитесь внутри Workspace (рабочего пространства).
...
```
2. Добавление конкретной версии crate:
...
<a name="q-031101"><h3>Что такое асинхронное программирование? Как оно связано с многопоточностью? Какие проблемы оно решает? Каковы предпосылки для его существования?</h3></a>
...
<a name="q-031102"><h3>Как работает неблокирующий ввод-вывод? Чем он отличается от блокирующего ввода-вывода?</h3></a>
...
<a name="q-031103"><h3>Что такое `Future`? Зачем он нам нужен? Как он работает в Rust и чем его семантика отличается от других языков программирования? Что делает его нулевым по стоимости?</h3></a>
...
<a name="q-031104"><h3>Что такое `async`/`.await`? Как они преобразуют `future` в `future`? Почему они так важны для эргономики?</h3></a>
...
<a name="q-031105"><h3>Что такое асинхронная задача? Чем она отличается от `Future`?</h3></a>
...
<a name="q-031106"><h3>Что такое `Waker`? Как он работает? Зачем он нужен?</h3></a>
...
<a name="q-031107"><h3>Что такое асинхронная среда выполнения? Из каких частей она обычно состоит?</h3></a>
...
<a name="q-031108"><h3>Какой тип многозадачности представлен `Future` в Rust? Какие у них преимущества и недостатки?</h3></a>
...
<a name="q-031109"><h3>Какие типы асинхронных сред выполнения существуют в Rust для многопоточности? Каковы преимущества и недостатки каждой из них?</h3></a>
...
<a name="q-031110"><h3>Почему блокировка в асинхронной среде выполнения — это плохо? Как этого избежать на практике?</h3></a>
...
<a name="q-031111"><h3>В чём заключаются ключевые моменты парадигмы параллельного выполнения в рамках модели акторов? Какую пользу она может принести в Rust?</h3></a>

Модель акторов в Rust — это подход к построению concurrent-приложений, где программа состоит из независимых изолированных сущностей — акторов . Каждый актор инкапсулирует своё собственное состояние и взаимодействует с другими акторами исключительно через асинхронный обмен сообщениями .

Этот подход особенно хорошо ложится на философию Rust, так как позволяет избегать блокировок и состояний гонки, перекладывая взаимодействие на каналы связи . Акторная модель в Rust чаще всего реализуется через специализированные фреймворки, которые берут на себя управление жизненным циклом акторов, их почтовыми ящиками и расписанием .
...
flowchart LR
    subgraph A1 [Актор А]
        direction TB
        S1[Состояние<br>State]
        MB1[("Почтовый ящик<br>(Mailbox)")]
    end
    
    subgraph A2 [Актор Б]
        direction TB
        S2[Состояние<br>State]
        MB2[("Почтовый ящик<br>(Mailbox)")]
    end
    
    MB1 -- "Асинхронное сообщение" --> MB2
    MB2 -- "Асинхронное сообщение" --> MB1
...
#### 1. Ключевые моменты парадигмы акторов
  ...
  - Изоляция состояния: Акторы не имеют общей памяти. У каждого актора есть свое приватное состояние, которое недоступно извне.
  - Слабая связанность (Loose Coupling): Акторы общаются только через асинхронную передачу сообщений. Вы не вызываете метод актора, вы отправляете ему «письмо» в его почтовый ящик (mailbox).
  - Адресация: Вы не владеете актором, вы владеете его адресом (Addr). Это позволяет актору упасть и перезапуститься, не ломая код отправителя.
  - Иерархия и надзор (Supervision): Акторы могут следить за своими «детьми». Если ребенок упал, родитель решает: перезапустить его, игнорировать или упасть самому.

#### 2. Какую пользу модель акторов приносит в Rust?

  Хотя Rust гарантирует безопасность памяти, управление сложным состоянием в многопоточной среде через `Arc<Mutex<T>>` часто превращается в «лапшу» из блокировок. Акторы решают это:

  1. Уход от блокировок (Lock-free логика)

      Внутри актора код выполняется последовательно. Вам не нужны Mutex или RwLock для защиты внутренних данных актора, так как сообщения обрабатываются по одному. Это устраняет риск взаимных блокировок (deadlocks).

  2. Упрощение жизненного цикла (Ownership)

      В Rust передача данных между потоками через каналы требует четкого владения. Модель акторов естественным образом ложится на систему владения Rust: сообщение «переезжает» (move) из одного почтового ящика в другой.

  3. Отказоустойчивость (Fault Tolerance)

      В обычном Rust-приложении panic! в одном потоке может быть трудно обработать системно. В акторных системах (например, Actix) паника актора просто приводит к уведомлению его супервизора, что позволяет строить самовосстанавливающиеся системы (в стиле Erlang/Elixir).

  4. Масштабируемость (Location Transparency)

      В развитых акторных системах неважно, находится ли актор в соседнем потоке или на другом сервере. Вы отправляете сообщение на адрес, а рантайм сам решает, доставить его через память или по сети.

#### Популярные решения в Rust (2026)

|Библиотека|Особенности|
|----------|-----------|
|Actix|Самая старая и проверенная система акторов в Rust. Очень быстрая.|
|Xactor|Более современный и компактный подход на базе async/await.|
|Bastion|Фокус на высочайшей отказоустойчивости и распределенности.|


#### Когда НЕ стоит использовать акторов?

Если ваша задача — простая параллельная обработка массива данных (например, умножение матриц), акторы добавят слишком много накладных расходов. В этом случае __Rayon__ будет гораздо эффективнее.

__Итог__: Модель акторов в Rust — это способ превратить сложную систему с общей памятью в набор независимых, общающихся друг с другом «микро-сервисов» внутри одного приложения.
