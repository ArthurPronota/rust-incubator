...
#### 2. Какую пользу модель акторов приносит в Rust?
  ...
  1. Уход от блокировок (Lock-free логика)
      ...
      Внутри актора код выполняется последовательно. Вам не нужны Mutex или RwLock для защиты внутренних данных актора, так как сообщения обрабатываются по одному. Это устраняет риск взаимных блокировок (deadlocks) [1.1].
