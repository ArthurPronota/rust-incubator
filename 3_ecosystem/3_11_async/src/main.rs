/*
    ...
    Запуск программы из лиректория rust-incubator/3_ecosystem/3_11_async:
        $ cargo run list_links.txt
...
use clap::Parser ;
// Определение количество ядер CPU
use num_cpus ;
// Использование стандартной библиотеки
use std::{
        // Тип char обозначает один символ.
        char,
        // Кроссплатформенное управление путями.
        path::{
            // Фрагмент пути (аналогично str).
            Path,
            // Собственный, изменяемый путь (аналогично строке).
            PathBuf
        }
    } ;
// Операции по манипулированию файловой системой.
use std::fs ;
// Асинхронные tokio задачи
use tokio::task ;
// HTTP клиент
use reqwest::Client ;
// Асинхронные потоки
...
// Субдиректорий для хранения содержимого страниц
...
async fn download_page(
            ...
            url: String
                ...
                Box<
                    dyn std::error::Error
                    // Send и Sync требуются, потому что ошибка передается
                    // между потоками в асинхронном контексте.
                    // Вместе они обеспечивают безопасную работу с ошибками в конкурентной
                    // среде
                        // тип можно передавать между потоками
                        + std::marker::Send
                        // на тип можно ссылаться из разных потоков (&T является Send)
                        + std::marker::Sync
                >
            > 
{
    ...
    let file_name = url
                                .replace("https://", "")
                                .replace("http://", "")
                                // Метод .replace() здесь используется в необычной форме
                                // с предикатом (функцией-условием), а не с конкретным паттерном.
                                // .replace() сам решает, как обойти строку — это его внутренняя реализация
                                // Он использует итерацию по символам (UTF-8)
                                // Предикат просто получает каждый символ по очереди                                
                                .replace(
                                    // для 'с' обязательно задать тип т.к. компилятор не может вывести 
                                    // тип параметра замыкания в данном контексте.
                                    |c: char| {
                                        !c.is_alphanumeric()
                                        && c != '.'
                                        && c != '-'
