...
/// Управление несколькими логгерами через разные target
...
impl MultiLogger {
    ...
    /// Добавляет новый логгер с уникальным target
    pub fn add_logger(&mut self, target: &str, path: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // сохраняем guard в массиве до завершения работы программы
        ...
        let subscriber = 
                ...
                tracing_subscriber::registry()
                    ...
                    .with(layer)
                    ;
        // Устанавливает этого подписчика в качестве глобального по умолчанию
...
fn main() {
    ...
    let mut logger = access_log::MultiLogger::new();

    if let Err(err) = logger.add_logger("access_log", "./access.log") {
        println!("err: {}", err) ;
    }

    if let Err(err) = logger.add_logger("error_log", "./error.log") {
        println!("err: {}", err) ;
    }
