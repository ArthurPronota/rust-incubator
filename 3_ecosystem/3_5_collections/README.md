...
- Возвращает IDs of Users, чей `nickname` содержит заданную строку (функция поиска).
...
<a name="q-030501"><h3>Что такое коллекция? Что такое итератор? Чем они отличаются? Как они используются? Какие ограничения есть у каждого из них?</h3></a>
...
- Lock-free алгоритмы: Некоторые коллекции используют __атомарные__ инструкции процессора (CAS — Compare-And-Swap) вместо мьютексов. Это позволяет изменять указатели в памяти без остановки других потоков.
