...
1. Значения по умолчанию объявляются непосредственно в исходном коде [Rust];
2. Значения считываются из TOML-файла;
3. Значения устанавливаются переменными окружения с префиксом `CONF_`.

Интерфейс командной строки [CLI] программы должен выглядеть следующим образом:
...
- [Почему переменные окружения полезны для настройки приложения? В чём их основное назначение?](#почему-переменные-окружения-полезны-для-настройки-приложения-в-чём-их-основное-назначение)
- [В чём польза crate `config`? Зачем его использовать, если нельзя просто десериализовать файл в структуры с помощью `serde`?](#в-чём-польза-crate-config-зачем-его-использовать-если-нельзя-просто-десериализовать-файл-в-структуры-с-помощью-serde)
...
### Почему переменные окружения полезны для настройки приложения? В чём их основное назначение?

Использование переменных окружения (Environment Variables) в Rust — это стандарт индустрии для создания гибких и безопасных приложений, соответствующих методологии [12-factor].

Это основной способ отделить код от конфигурации среды.

#### Основное назначение переменных окружения

Главная цель — __динамическая настройка без пересборки и перезаливки кода__.
Переменные окружения позволяют одному и тому же бинарному файлу Rust вести себя по-разному в зависимости от того, где он запущен: на ноутбуке разработчика, в тестовой среде (Staging) или в облаке (Production).

#### Почему они полезны в Rust?

1. Безопасность секретов (Secrets Management)

Это критически важно: __никогда не храните пароли и API-ключи в коде или в конфигурационных файлах Git__.

- В Rust: Вы читаете секрет из окружения через std::env::var("DATABASE_URL"). Сами данные подставляются системой управления секретами (например, HashiCorp Vault или Kubernetes Secrets) прямо в память процесса.

2. Нативная поддержка Docker и Kubernetes

Облачные платформы спроектированы для работы с переменными окружения.

- Удобство: Вам не нужно пробрасывать файлы конфигурации внутрь контейнера через разделы (Volumes). Достаточно указать -e PORT=8080 при запуске.

3. Переопределение "на лету"

Переменные окружения имеют самый высокий приоритет. Даже если в вашем config.toml прописан уровень логов INFO, вы можете запустить приложение с RUST_LOG=debug, чтобы быстро разобраться в проблеме, не меняя файлы.

4. Интеграция с экосистемой логов

Многие стандартные крейты Rust (например, env_logger или tracing-subscriber) «из коробки» умеют настраиваться через переменные окружения.

#### Как это выглядит в коде

Для удобной работы в Rust принято использовать крейт dotenvy (современный форк dotenv).
...
fn main() {
    ...
    // Читаем переменную. Если её нет — используем значение по умолчанию
...
|Характеристика|Config файл (TOML/JSON)|Переменные окружения (ENV)|
|--------------|-----------------------|--------------------------|
|Для чего лучше|Сложные иерархические настройки|Секреты, флаги, адреса сервисов|
|Обновление|Нужно менять файл / перезапуск|Меняются в оркестраторе (K8s)|
|Безопасность|Риск попасть в Git|Максимальная безопасность|

__Итог__: Переменные окружения делают ваше Rust-приложение портативным и безопасным.

<hr>

### В чём польза crate `config`? Зачем его использовать, если нельзя просто десериализовать файл в структуры с помощью `serde`?

Польза крейта config в Rust заключается в многослойности (Layering). Обычная десериализация через serde работает по принципу «один файл — одна структура», в то время как config позволяет собрать финальную конфигурацию из множества источников, накладывая их друг на друга.

#### Зачем использовать [`config`], если есть [`serde`]?

1. Слияние источников (Merging)

Это главная «киллер-фича». В реальном приложении настройки хранятся в разных местах. Крейт config позволяет сделать так:

1. Загрузить базовый default.toml.
2. Наложить сверху production.toml (переопределив только нужные поля).
3. Наложить сверху переменные окружения (например, пароли из секретов Kubernetes).
4. Наложить аргументы командной строки.

__Результат__: Одна структура, собранная из 4 источников [1.1].

2. #### Динамическое переопределение (Overrides)

С помощью [`config`] вы можете жестко задать параметры прямо в коде перед парсингом, что удобно для тестов или специфических условий запуска, которые не должны храниться в файлах.

3. #### Поддержка разных форматов одновременно

Крейт умеет «под капотом» работать с JSON, TOML, YAML, INI и даже RON в рамках одного билдера. Вам не нужно подключать serde_json, toml и serde_yaml по отдельности и писать логику их объединения.

4. #### Работа с префиксами ENV

Он позволяет автоматически маппить переменные окружения типа APP_DATABASE__URL в поле database.url вашей структуры, соблюдая иерархию. [`serde`] сам по себе так не умеет — он ожидает готовый плоский или вложенный объект.

#### Практическое сравнение

|Задача|Только Serde|Crate config|
|------|------------|------------|
|Прочитать один файл|Идеально и быстро.|Избыточно.|
|Пароль в ENV, порт в TOML|Нужно писать код слияния вручную.|Делается одной строкой .add_source().|
|Иерархия настроек|Сложно объединять Map.|Автоматическое слияние деревьев.|
...
let settings = Config::builder()
    // 1. Берем дефолты из файла
    .add_source(File::with_name("config/default"))
    // 2. Добавляем настройки конкретной среды (dev/prod)
    .add_source(File::with_name("config/local").required(false))
    // 3. Переопределяем через переменные окружения (напр. APP_PORT)
    .add_source(Environment::with_prefix("APP").separator("__"))
    .build()
    .unwrap();

// И только в самом конце мы превращаем это в вашу структуру через Serde
let config: MyConfig = settings.try_deserialize().unwrap();
```

__Итог__: Используйте serde напрямую, если у вас один конфиг-файл. Используйте config, если вы строите облачное приложение (Cloud-Native), где важна гибкость настройки под разные среды без изменения кода.
