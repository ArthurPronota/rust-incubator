// запуск тестов:   cargo test -- --nocapture
...
pub struct FormatSpec<'a> {
    ...
    pub precision:  Option<(&'a str, Option<&'a str>)>,
    pub ty: Option<&'a str>,
}
...
fn parse_format_spec<'a>(input: &mut &'a str) ->
        Result<FormatSpec<'a>, ErrMode<ContextError>>
 */
{
    ...
    if let Some(_) = opt(
        ...
        spec.fill = Some(any // это парсер, который распознает любой один символ из входных данных.
                            .parse_next(input)?
                        );

        spec.align = Some(one_of( // это парсер который распознает один символ из заданного набора символов.
                        align_chars
                        ).parse_next(input)?
                    );
    } else {
        // Если первого символа нет, возможно есть только align
        spec.align = opt(   // этот комбинатор делает парсер опциональным, если парсер не срабатывает возвращает None
                one_of( // это парсер который распознает один символ из заданного набора символов.
                                //align_chars
                                ['<','>','^']
                            )).parse_next(input)?;
    ...
    // 2. [sign] (+ или -)
    spec.sign = opt(    // этот комбинатор делает парсер опциональным, если парсер не срабатывает возвращает None
            one_of( // это парсер который распознает один символ из заданного набора символов.
                        ['+', '-']
                    )
                )
