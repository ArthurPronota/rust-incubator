...
- [Когда имеет смысл предпочесть `musli` вместо `serde`?](#когда-имеет-смысл-предпочесть-musli-вместо-serde)

- [Что такое десериализация с нулевым копированием? Почему она полезна? Как она работает в `serde`? Как она работает в `rkyv`?](#что-такое-десериализация-с-нулевым-копированием-почему-она-полезна-как-она-работает-в-serde-как-она-работает-в-rkyv)
...
### Когда имеет смысл предпочесть `musli` вместо `serde`?

Хотя Serde — это «золотой стандарт» Rust, проект musli (MUlti-Size Library) занял нишу, где ограничения Serde становятся критичными.

Переход на musli оправдан в следующих сценариях:

1. ####  Экстремально ограниченные ресурсы (Embedded / no_std)

_Serde_ часто требует значительного объема памяти для генерации кода и работы _Visitor_.

- __Преимущество musli__: Он спроектирован для систем с жестким лимитом памяти. Он более эффективно работает в средах без аллокатора (alloc), предлагая более легкие абстракции для сериализации.

2. #### Нужна максимальная скорость компиляции макросов

Макросы _serde_ (derive) известны тем, что значительно замедляют сборку больших проектов, так как они генерируют много сложного кода для каждого типа.

- Преимущество _musli_: Он использует другой подход к кодогенерации, который стремится быть менее «тяжелым» для компилятора Rust. Если у вас сотни структур и время сборки стало проблемой — это весомый аргумент.

3. #### Сложные или нестандартные форматы данных

Serde основан на фиксированной «модели данных» (Serde Data Model). Если ваш формат не вписывается в неё (например, требует передачи контекста или специфических метаданных на глубоких уровнях вложенности), вам приходится использовать «костыли» вроде seed или thread_local.

- __Преимущество musli__: Он позволяет передавать контекст через все уровни сериализации «из коробки». Это делает его идеальным для проприетарных бинарных протоколов со сложной логикой версионирования.

4. #### Улучшенная поддержка Zero-copy

Хотя Serde поддерживает zero-copy, в musli этот механизм является «первоклассным гражданином».

- __Преимущество musli__: Он предлагает более интуитивные инструменты для работы с заимствованными данными, особенно когда структура данных и её бинарное представление должны быть максимально близки друг к другу по расположению в памяти.

#### Сводная таблица сравнения

|Критерий|Serde|Musli|
|--------|-----|-----|
|Экосистема|Огромная (все библиотеки поддерживают)|Маленькая (нишевая)|
|Гибкость модели|Фиксированная (Serde Data Model)|Настраиваемая|
|Время компиляции|Часто медленное|Быстрее|
|Сложность обучения|Низкая (много примеров)|Выше (меньше ресурсов)|

#### Резюме: Когда делать выбор?

- Оставайтесь на Serde, если вам нужна совместимость с JSON, TOML, SQLx, Postgres и 99% других библиотек. Это самый безопасный выбор для веба и обычных приложений.
- Переходите на musli, если вы пишете высокопроизводительный бинарный протокол, работаете в Embedded (микроконтроллеры) или если время компиляции проекта на Serde стало неприемлемым.

<hr>

#### Что такое десериализация с нулевым копированием? Почему она полезна? Как она работает в `serde`? Как она работает в `rkyv`?

Десериализация с нулевым копированием (Zero-copy deserialization) — это техника, при которой данные из входного буфера (байтов) не копируются в новые структуры в куче, а заимствуются напрямую.

В Rust это считается «святым граалем» производительности для высоконагруженных систем и Embedded-разработки.

#### Почему это полезно?

- Скорость: Нет затрат на копирование байтов из одного места в памяти в другое.
- Экономия памяти: Вы не дублируете данные. Если у вас в буфере строка на 1 ГБ, вам не нужно еще 1 ГБ в куче для её хранения.
- Меньшая нагрузка на аллокатор: Нет постоянных выделений и освобождений памяти (malloc/free), что снижает фрагментацию.

#### Как это работает в serde?

_Serde_ достигает этого через заимствование из входного буфера.

Когда вы десериализуете поле как _&str_ или _&[u8]_, Serde не создает новую String. Вместо этого он создает ссылку, которая указывает прямо на байты внутри исходного массива.

- Механизм: Используются времена жизни (lifetimes), обычно обозначаемые как 'a. Структура данных помечается как «живущая не дольше, чем исходный буфер».
- Ограничение: Формат данных должен поддерживать это. Например, в JSON строки часто экранированы ("path": "C:\\Users"). Чтобы получить &str, Serde придется «разэкранировать» строку, а это требует выделения новой памяти. В таких случаях zero-copy невозможен.
...
#[derive(Deserialize)]
struct User<'a> {
    #[serde(borrow)] // Явное указание на заимствование
    name: &'a str, 
}
```

#### Как это работает в rkyv?

rkyv (произносится как «archive») использует принципиально иной подход — Zero-parsing.

Если _Serde_ всё еще нужно «пройтись» по JSON или Bincode, чтобы найти границы полей (парсинг), то _rkyv_ хранит данные в памяти в том же виде, в котором они используются в Rust (или максимально близком).

- Архивация: При записи rkyv выравнивает данные и разрешает указатели (превращая их в относительные смещения).
- Доступ: Чтобы «прочитать» данные, вам не нужно их десериализовать вообще. Вы просто делаете memory-map файла в память и приводите указатель к типу ArchivedT.
- Механизм: Вы работаете с «архивной» версией структуры. Доступ к полям происходит мгновенно, как если бы это была обычная структура в памяти.

#### Сравнение

|Критерий|Serde (Zero-copy)|rkyv (Zero-parsing)|
|--------|-----------------|-------------------|
|Скорость доступа|Высокая (нужен парсинг)|Мгновенная (парсинг не нужен)|
|Сложность|Средняя (времена жизни)|Высокая (архивные типы)|
