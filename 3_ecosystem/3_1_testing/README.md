...
где "10" - число которое нужно угадать.

> Особенности тестов для данной программы.
>
> Модульные тесты в данном случае создать невозможно, т.к. Rust не позволяет:
> - изменить аргументы командной строки
> - напрямую писать в std::io::stdin вашего собственного процесса (есть обход для linux но его стоит избегать)
>
> Поэтому создаём четыре __Интегральных__ теста проверяющих критические места программы.
>
...
- [`Что такое fuzzing? Чем он отличается от проверки свойств?`](#%D1%87%D1%82%D0%BE-%D1%82%D0%B0%D0%BA%D0%BE%D0%B5-fuzzing-%D1%87%D0%B5%D0%BC-%D0%BE%D0%BD-%D0%BE%D1%82%D0%BB%D0%B8%D1%87%D0%B0%D0%B5%D1%82%D1%81%D1%8F-%D0%BE%D1%82-%D0%BF%D1%80%D0%BE%D0%B2%D0%B5%D1%80%D0%BA%D0%B8-%D1%81%D0%B2%D0%BE%D0%B9%D1%81%D1%82%D0%B2)

<hr>

<a name="q-030101"><h3>Что такое стиль TDD? Что такое стиль BDD? В чём заключается основной акцент в стиле BDD?</h3></a>
...
<a name="q-030102"><h4>Что такое mocking (имитация)? Когда она полезна?</h4></a>
...
<a name="q-030103"><h3>Что такое проверка свойств? Как она достигает своих целей?</h3></a>

Проверка свойств (`Property-Based Testing`, `PBT`) — это метод тестирования, при котором вместо написания конкретных входных данных (как в обычном `TDD`), вы описываете инварианты (свойства), которые должны оставаться истинными для любых входных данных.

В Rust этот подход считается «высшим пилотажем» обеспечения качества, особенно в криптографии, протоколах и системном коде.
...
Вы формулируете свойство:

__«Если я переверну любой вектор дважды, я должен получить исходный вектор обратно».__

<h4>Как она достигает своих целей?</h4>

Инструменты (в Rust это стандарт де-факто — `proptest` или `quickcheck`) используют следующие механизмы:

1. Генерация случайных данных: Библиотека генерирует сотни и тысячи случайных сценариев (граничные значения, пустые строки, огромные числа, специфические символы Unicode), которые человек никогда не придумал бы вручную.
2. Шринкинг (`Shrinking`/Усадка): Это «киллер-фича» `PBT`. Если библиотека находит входные данные (например, строку из 1000 символов), на которых тест падает, она начинает автоматически упрощать их до минимально возможного примера (например, строки из 1 символа), который всё еще вызывает ошибку. Это дает разработчику готовый отчет о баге в чистом виде.
3. Поиск логических дыр: `PBT` находит условия, при которых инварианты нарушаются (например, переполнение целого числа или потеря точности при округлении), что практически невозможно поймать обычными юнит-тестами.

<h4>Пример на proptest:</h4>

```rust
/* 
    Импортирует необходимые макросы и инструменты для 
    генерации случайных данных. 
*/
use proptest::prelude::*;

// Специальный макрос, который превращает обычную функцию в цикл фаззинга. 
...
proptest! {
    ...
    fn test_string_reversal(
            /*
in ".*": Это стратегия генерации. Регулярное выражение ".*" говорит библиотеке 
генерировать абсолютно любые строки (пустые, с пробелами, спецсимволами, Unicode-эмодзи и т.д.).            
             */
            s in ".*"
        ) 
    {
...
<h4>Магия шринкинга (Shrinking)</h4>

Представь, что тест упал на огромной строке из 1000 символов. Читать её невозможно. proptest начнет автоматически упрощать эту строку:

1. Попробует строку из 500 символов.
2. Попробует из 10 символов.
3. В итоге выдаст тебе минимально возможный пример, на котором ломается логика (например, конкретный символ Unicode, который занимает несколько байт).


<h4>Когда это полезно?</h4>

- Парсеры: Подайте на вход любые случайные байты — парсер не должен паниковать.
- Бизнес-логика: Например, при любых транзакциях сумма на балансах должна оставаться неизменной.
- Сериализация: Объект -> JSON -> Объект должен давать идентичный результат.
...
<a name="q-030104"><h3>Что такое fuzzing? Чем он отличается от проверки свойств?</h3></a>

В Rust __fuzzing__ (фаззинг) — это метод автоматизированного тестирования, который подает на вход программе огромные массивы псевдослучайных, зачастую некорректных или «мусорных» данных с целью вызвать падение (panic), зависание или повреждение памяти.

Основными инструментами являются `cargo-fuzz` (обертка над `libFuzzer`) и `AFL++`.

<h4>Ключевые отличия от проверки свойств (`Property-Based Testing`)</h4>

Хотя оба метода генерируют случайные данные, они преследуют разные цели и работают по-разному:

|Характеристика|Проверка свойств (Proptest)|Фаззинг (libFuzzer / AFL)|
|--------------|---------------------------|-------------------------|
|Цель|Проверка логики и инвариантов.|Поиск краш-тестов, утечек и уязвимостей.|
|Входные данные|Структурированные (числа, строки).|Обычно сырые байты (`&[u8])`.|
|Умность|Генерирует данные по правилам (стратегиям).|Управляется покрытием кода (`coverage-guided`).|
|Время работы|Секунды (в составе cargo test).|Часы или дни (отдельный процесс).|
|Обратная связь|Не знает, как работает код внутри.|Анализирует, какие ветки кода (`if/else`) были затронуты.|

<h4>Как работает фаззинг (механизм)</h4>

Основное преимущество фаззинга в Rust — анализ покрытия. Фаззер «наблюдает», по какому пути пошло выполнение программы при определенных байтах. Если новый набор байтов заставил программу зайти в ранее не посещенную ветку else, фаззер сохраняет этот набор как «интересный» и продолжает мутировать его, чтобы зайти еще дальше.

<h4>Когда использовать фаззинг?</h4>

1. Парсеры: Если вы принимаете данные извне (`JSON`, изображения, сетевые пакеты). Фаззинг — лучший способ убедиться, что битый файл не «уронит» сервер.
2. `Unsafe-код`: Если вы работаете с сырой памятью. Фаззеры эффективно находят сегфолты и ошибки доступа.
3. Криптография: Для поиска редких математических аномалий.

<h4>Пример типичной цели фаззинга:</h4>

```rust
// Находится в fuzzer/fuzz_targets/target1.rs

/* директива сообщает компилятору Rust, что в файле не 
будет стандартной функции main
*/
#![no_main]
/*
Импорт макроса, который связывает ваш код с низкоуровневой библиотекой
фаззинга. Она отвечает за генерацию "умных" случайных данных
*/
use libfuzzer_sys::fuzz_target; 
/*
Это бесконечный цикл. Фаззер будет вызывать это замыкание тысячи раз
в секунду.
*/
fuzz_target!(|
/*
Это "сырые" байты, генерируемые фаззером. Они могут быть любой длины
и содержать абсолютно любой мусор.
*/
            data: &[u8]
            | {
    // Мы пытаемся распарсить любые случайные байты
    if let Ok(config) = my_library::parse_config(data) // парсер успешно справился с мусором и выдал объект
    {
        /*
            проверяет цепочку зависимостей: корректные данные после
            парсинга должны быть корректными и для валидатора
        */
        let _ = config.validate();
    }
    // Если parse_config упадет в панику — фаззер найдет эти байты и сохранит их
});
```

<h4>Как это работает "под капотом"?</h4>

- Coverage-guided: Фаззер не просто кидает случайные байты. Он "смотрит", какие ветки вашего кода были затронуты. Если определенный набор байтов заставил программу зайти в новый else блок, фаззер сохранит этот набор (корпус) и будет мутировать уже его, чтобы зайти ещё глубже в логику.
- Поиск паник: Если внутри parse_config случится panic!, фаззер немедленно остановится, сохранит эти байты в файл `crash-*` и покажет вам точный отчет.
- Sanitizers: Обычно такой код запускается с "санитайзерами" (`AddressSanitizer`), которые моментально ловят ошибки работы с памятью, даже если вы используете `unsafe`.

Если ваша программа принимает данные из интернета, из файлов или от пользователя, этот крошечный фрагмент кода может найти баги, которые не нашли бы сотни обычных тестов.


<h4>Итог:</h4>
Если `Proptest` — это продвинутый юнит-тест для проверки «правильно ли я считаю», то `Fuzzing` — это испытание на прочность: «выживет ли мой код под атакой или при чтении битых данных».
