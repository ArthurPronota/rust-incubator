...
- Обязанность: Это встроенная библиотека компилятора. Она предоставляет базовые типы `TokenStream` и `Span`.
- Почему обязателен: Без него вы не сможете объявить функцию как макрос.
- Это «клей» между вашим кодом и компилятором rustc.
Нюанс: Доступен только в крейтах с пометкой __proc-macro = true__ в __Cargo.toml__.
