...
|-----------|-----|-------------|------------------|
|Function-like|my_macro!()|Генерация кода с нуля или DSL(domain-specific language).|Нет (создает новое)|
|Derive|`#[derive(Trait)]`|Реализация трейтов.|Нет (только добавляет)|
|Attribute|`#[my_attr]`|Модификация логики функций/типов.|Да (может заменить)|
