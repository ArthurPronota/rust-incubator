...
- [CLI] клиент ничего не делает, кроме как отправляет команды «как есть» на сервер и отображает его ответы.
- Сервер [демон][52], имеющий единственную [HTTP] конечную точку, выполняет весь анализ и выполнение команд, отправленных [CLI] клиентом.
...
В экосистеме Rust выбор HTTP-клиента зависит от того, нужен ли вам комфорт «всё включено» или максимальная легкость. Вот основные игроки:

1. reqwest (Стандарт индустрии)

Самый популярный и универсальный крейт. Построен на базе hyper и tokio.

- Плюсы: Поддерживает всё (JSON, multipart, cookies, прокси, WASM). Есть и async, и blocking режимы.
- Компромиссы: Тяжелый (много зависимостей). Если вам нужно просто сделать один GET-запрос, reqwest притащит с собой половину экосистемы Tokio.

2. ureq (Минимализм и простота)

Лучший выбор, если вам не нужен async.

- Плюсы: Работает только в блокирующем режиме (синхронно). Маленький размер, быстрая компиляция, минимум зависимостей. Идеально для CLI-утилит.
- Компромиссы: Не подходит для высоконагруженных серверов, где нужно обрабатывать тысячи параллельных запросов (так как каждый запрос блокирует поток).

3. hyper (Низкоуровневый фундамент)

Это «двигатель», на котором ездят другие крейты.

- Плюсы: Максимальная производительность и гибкость. Вы контролируете каждый байт и каждое событие в соединении.
- Компромиссы: Сложный API. Чтобы просто отправить JSON, придется написать много шаблонного кода. Обычно его не используют напрямую для простых задач.

4. surf (Часть экосистемы async-std)

Альтернатива reqwest, ориентированная на модульность.

- Плюсы: Гибкая система плагинов (middleware). Может работать с разными рантаймами (не только Tokio).
- Компромиссы: Менее популярен, чем reqwest, обновления выходят реже.

5. awc (Клиент для Actix-web)

Если вы уже пишете сервер на actix-web, логично использовать этот клиент.

- Плюсы: Отличная интеграция с экосистемой Actix, очень высокая скорость.
- Компромиссы: Плохо подходит для использования вне проектов на Actix.

#### Сводная таблица выбора

|Если нужно...|Используйте|Почему|
|-------------|-----------|------|
|Просто и быстро (async)|reqwest|Есть всё, огромное сообщество.|
|Маленький бинарник (CLI)|ureq|Нет async, мало зависимостей.|
|Максимум скорости / Свой протокол|hyper|Полный контроль над HTTP.|
|Работа в браузере (WASM) (интерфейс написан на Rust, а реализация (выполнение) делегируется браузеру)|reqwest или gloo-net|	Хорошая поддержка JS-окружения.|

__На заметку:__ Если вы используете __reqwest__, не забудьте отключить дефолтные фичи (например, __default-features = false__), чтобы не тянуть лишние библиотеки для шифрования (OpenSSL), если вам достаточно rustls.

<hr>

<a name="q-4204"><h3>Что такое WebSocket? Как он используется и когда? В двух словах, как он работает?</h3></a>

WebSocket в Rust — это __технология, позволяющая открыть постоянное двухстороннее соединение между клиентом и сервером через один TCP-порт__.

#### В двух словах: Как он работает?

1. Handshake: Клиент отправляет обычный HTTP-запрос с заголовком Upgrade: websocket.
2. Переключение: Если сервер согласен, они «договариваются» больше не использовать HTTP.
3. Туннель: Соединение остается открытым. Теперь и клиент, и сервер могут в любой момент отправить пакет данных (фрейм) друг другу без лишних заголовков и ожидания запроса.

#### Когда это используется?

- Real-time чаты: Сообщение должно прийти мгновенно, не дожидаясь, пока клиент обновит страницу.
- Биржи и котировки: Данные о ценах меняются несколько раз в секунду.
- Мультиплеерные игры: Нужно постоянно синхронизировать координаты игроков.
- Уведомления: Сервер сам «толкает» (push) инфо на клиент, когда что-то произошло.

#### Популярные крейты в Rust

В Rust работа с WebSocket обычно строится поверх асинхронных рантаймов:

1. tungstenite: Самая популярная библиотека. Реализует сам протокол WebSocket.
    - Есть версия tokio-tungstenite для работы внутри Tokio.
2. fastwebsockets: Очень быстрая альтернатива (используется, например, в Deno).
3. axum / actix-web: У этих веб-фреймворков есть встроенная поддержка или официальные модули для превращения HTTP-запроса в WebSocket-сессию.

#### Как это выглядит в коде (концептуально)

В Rust WebSocket часто представляют как __Stream__ (поток входящих сообщений) и __Sink__ (куда можно отправлять сообщения):

```rust
// Упрощенный пример с tokio-tungstenite
while let Some(msg) = ws_stream.next().await {
    let msg = msg?;
    if msg.is_text() {
        // Сервер получил текст и тут же отправил ответ
        ws_stream.send(Message::Text("Понял, принял".into())).await?;
    }
}
```
...
- Плюс: Минимальные задержки и экономия трафика (нет тяжелых HTTP-заголовков в каждом пакете).
- Минус: Серверу нужно держать соединение открытым (Stateful). Если у вас 100 000 активных пользователей, вам нужно 100 000 открытых сокетов в памяти, что требует правильной настройки ОС и Rust-рантайма.
