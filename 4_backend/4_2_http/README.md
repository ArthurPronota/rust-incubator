...
- Влияние на код: Вы пишете обычный ___async/await___. Данные между потоками должны быть ___Send + Sync___. Вам часто нужны ___Arc<Mutex<T>>___ или `Arc<RwLock<T>>`, так как задача может начаться на одном ядре, а продолжиться на другом.
