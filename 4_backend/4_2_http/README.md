...
__Итог: Tokio автоматически создает пул потоков по числу ядер и эффективно перекидывает задачи между ними, но финальное решение, на каком именно физическом ядре сейчас работает поток, обычно остается за ОС.
