...
- Влияние на код: Вы пишете обычный ___async/await__. Данные между потоками должны быть ___Send + Sync___. Вам часто нужны `Arc<Mutex<T>>` или `Arc<RwLock<T>>`, так как задача может начаться на одном ядре, а продолжиться на другом.
